...
\pubyear{2023}
\copyrightyear{2023}
\externaleditor{Academic Editor: }%MDPI: Please add if necessary. % Authors: we don't know Academic Editor yet.
...
%$^{2}$ \quad olivier.debeir@ulb.be}%MDPI: Please add. % Authors: we are from the same affiliation, we have only one affiliation, so I deleted the 2nd.
...
The study region is Japan, shown in Figure \ref{fig01}. 
...
Afterwards, the number %MDPI: Please check indent is necessary. % Authors: yes, correct, the indent is done to highlight the code snippets.
...
Now, when the color palette was adjusted to the dataset, the map of prefectures of Japan was colored by assigning each individual color to each of the 47 prefectures, as follows: % Authors: the same: the indent is done to highlight the code snippets.
\begin{verbatim}
	scale_fill_manual(values = getPalette(colourCount)).
\end{verbatim} 
% Authors: the same: the indent is done to highlight the code snippets.
Other, additional cartographic elements included adding the titles, subtitles and captions by the ‘labs’ function, which enables the modification of the labels, annotations and captions. The rest of the aesthetics were added using $theme()$ function, as shown in Figure \ref{fig04}, which provides an illustration of the script on the left and the resulting output map on the right part of the menu. These include, among others, plotting the legend, its orientation and annotations, defining the text size and font characteristics, selecting the color of background, and choosing the types of grid ticks and the frequency of breaks on a cartographic grid. The final map is shown in Figure \ref{fig04}. 
...
\caption{Mapping the prefectures of Japan. (\textbf{a}) Script used for mapping by ‘ggplot2’, ‘ggmap’, ‘maps’ and ‘mapdata’ packages. (\textbf{b}) Map of the prefectures of Japan. Source: authors. \label{fig04}}%There is no (a,b) in the image. Please add. % Authors: corrected.
\end{figure}   
...
\makebox[0.90\linewidth][c]{%
	\includegraphics[width=18cm]{Fig-06.jpg}
}
\caption{Japan: 3D topographic mesh model with isolines drawn on top of surface based on ETOPO2, with rotation of 115/30\textdegree. Software: GMT, version 6.1.1. Map source: authors. \label{fig06}}%Some numbers are overlapping. Please modify. % Authors: corrected. The 3D map in Figure 6 is replotted: we decreased the font size and increased the step of annotations to 4000 m. 
\end{figure}   
...
\begin{figure}[H]
\makebox[0.90\linewidth][c]{%
	\includegraphics[width=17cm]{Fig-07.jpg}
}
\caption{Japan: 3D grayshaded topographic waterfall plot based on ETOPO2 with rotation of 65/30\textdegree . Software: GMT, version 6.1.1. Map source: authors. \label{fig07}}%Some numbers are overlapping. Please modify. % Authors: corrected. The map in Figure 7 is replotted with changed font size and step of annotations on vertical scale.
\end{figure}  
...
\end{figure}   %There is no (a,b) in the image. Please add. % Authors: corrected.
...
\caption{Aspect exposure of slopes in Japan. (\textbf{a}) Script by ‘raster ’ and ‘tmap’ packages. (\textbf{b}) Map prepared in RStudio. Source: authors. \label{fig09}}%There is no (a,b) in the image. Please add. % Authors: corrected.
\end{figure}   
...
\caption{Terrain hillshade in Japan. (\textbf{a}) Script used for plotting using ‘raster ’ and ‘tmap’ packages of R. (\textbf{b}) Map in RStudio. Source: authors. \label{fig10}}%There is no (a,b) in the image. Please add. % Authors: corrected.
\end{figure}   
...
\begin{figure}[H]
\centering
	\includegraphics[width=11.0 cm]{Fig-11}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\supplementary{The following are available online at \linksupplementary{s1}, Figure S1: title, Table S1: title, Video S1: title.}%MDPI: Please add. we noticed the supplementary file is duplicate as appendix. Please modify. % Authors: yes, the supplementary file includes the same scripts as in Appendix. To avoid duplication, the the supplementary file can be deleted while the Appendix remained. For convenience of the reader, it is better to have a reference with scripts directly in the Appendix. 
...
ENVI GIS & ENvironment for Visualizing Images GIS \\
...
\renewcommand{\thelstlisting}{A\arabic{lstlisting}}%MDPI: Please confirm if the bold is unnecessary and can be removed. The following highlights are the same. % Authors: the bold is removed in all  cases.
\begin{lstlisting}[language=bash,caption=GMT script for topographic map.]
#!/bin/sh
...
\begin{lstlisting}[language=bash,caption=GMT script for 3D surface plot of Japan.]
#!/bin/sh
...
\begin{lstlisting}[language=bash,caption=GMT script for 3D mesh model of Japan.]
#!/bin/sh
# Add 3D
...
\begin{lstlisting}[language=bash,caption=GMT script for 3D grayscale `waterfall' model of Japan.]
#!/bin/sh
...
\begin{lstlisting}[language=R, caption=R script for plotting prefectures of Japan.]
setwd("/Users/pauline/")
# load packages 
library(showtext)
library(ggplot2)
...
\begin{lstlisting}[language=bash,caption=R script for terrain maps.]
...
\begin{lstlisting}[language=bash,caption=R script for mapping slope map of Japan.]
# Set working directory
setwd("/Users/pauline/")
# load packages
library(sp)
...
\begin{lstlisting}[language=bash,caption=R script for mapping aspect of the terrain in Japan.]
# Aspect
tmap_mode("plot")
map2 <-
    tmap_style("albatross"
...
\begin{lstlisting}[language=bash,caption=R script for mapping hillshade in the terrain of Japan.]
# hillshade
# tmaptools::palette_explorer()
tmap_mode("plot")
map3 <-
...
\begin{lstlisting}[language=bash,caption=R script for mapping elevation heights over Japan.]
# elevation
tmap_mode("plot")
map4 <-
    tmap_style("white") +
...
%\newblock Geomorphology in the Geocomputing Landscape: GIS, DEMs, Spatial Analysis and statistics, %MDPI: Is this necessary? Can this be deleted? % Authors: not necessary, we deleted it.
...
%\newblock The Binghamton Geomorphology Symposium: 50 years of Enhancing Geomorphology, % Authors: not necessary, we deleted it.
...
%\newblock Ecohydrology for harmonization of societal needs with the biosphere potential,  % Authors: not necessary, we deleted it.
  {{https://doi.org/10.1016/j.ecohyd.2013.03.004}}.
...
%\newblock The Binghamton Geomorphology Symposium: 50 years of Enhancing Geomorphology % Authors: not necessary, we deleted it.
 {{https://doi.org/10.1016/j.geomorph.2019.06.019}}.
...
%\newblock Tectonic Evolution of Korean Peninsula and Adjacent Crustal Fragments in Asia, % Authors: not necessary, we deleted it.
  {{https://doi.org/10.1016/j.gr.2005.03.001}}.
...
\newblock RStudio Inc., Boston, MA, USA, 2017.  Available online: \url{https://www.RStudio.com/}  (accessed on 23 December 2022).%MDPI: Please add the access date (format: Date Month Year), e.g., accessed on 1 January 2020. % Authors: corrected.

\bibitem[Hijmans(2017)]{Hijmans}
Hijmans, R.J.
\newblock {raster: Geographic Data Analysis and Modeling. R Package Version
  2.6-7}.  2017.
\newblock Available online: \url{https://CRAN.R-project.org/package=raster}  (accessed on 23 December 2022).%MDPI: Please add the access date (format: Date Month Year), e.g., accessed on 1 January 2020. % Authors: corrected.
...
%\newblock Understanding earth surface processes from remotely sensed digital terrain models, % Authors: not necessary, we deleted it.
  {{https://doi.org/10.1016/j.geomorph.2009.06.004}}.
...
\newblock maps: Draw Geographical Maps. R Package Version 2.3-2. 2013. Available online: \url{http://CRAN.R-project.org/package=maps}  (accessed on 23 December 2022).%MDPI: Please add the access date (format: Date Month Year), e.g., accessed on 1 January 2020. % Authors: corrected.
...
%\newblock Contributions to Temperate and Humid Tropical Palaeohydrology, % Authors: not necessary, we deleted it.
...
%\newblock Recent advances in landslide investigation, % Authors: not necessary, we deleted it.
...
%\newblock Mountain Rivers Part I: Watershed Scale Processes and Channel Morphology, % Authors: not necessary, we deleted it.
  {{https://doi.org/10.1016/j.geomorph.2006.01.018}}.
